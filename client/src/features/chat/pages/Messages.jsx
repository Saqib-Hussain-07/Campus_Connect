...
export default function Messages() {
    ...
    const fetchConversations = () => {
      fetch('/api/messages/conversations', {
        ...
        .then((data) => {
          ...
          // If no active chat with ID specified in URL, set the first partner from list
  ...
  // Fetch thread for active conversation partner
  useEffect(() => {
    if (!token || !activeWithId) return;

    const fetchThread = () => {
      fetch(`/api/messages/thread/${activeWithId}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
        .then((res) => {
    ...
    fetchThread();
    const interval = setInterval(fetchThread, 5000); // poll thread faster
    return () => clearInterval(interval);
  }, [token, activeWithId]);
